...
default. You'll have to add the "pyramid_beaker" package as a dependency, and
put the following line in your ``main()`` function::
...
To use file-based sessions like in Pylons, uncomment the first three lines in
the session configuration and comment out the "session.type = memory" line.
...
the ability to scale to a multi-server cluster. 
